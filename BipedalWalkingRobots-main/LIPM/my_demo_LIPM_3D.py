...
for i in range(int(total_time/delta_t)):
    ...
    if step_num >= 1:
        if LFPC_model.support_leg is 'left_leg':
            LFPC_model.right_foot_pos = [swing_foot_pos[j,0], swing_foot_pos[j,1], swing_foot_pos[j,2]]
        else:
            LFPC_model.left_foot_pos = [swing_foot_pos[j,0], swing_foot_pos[j,1], swing_foot_pos[j,2]]
        j += 1
    ...
    if (i > 0) and (i % switch_index == 0):
        ...
        if LFPC_model.support_leg is 'left_leg':
            ...
            swing_foot_pos[:,0] = np.linspace(LFPC_model.right_foot_pos[0], right_foot_target_pos[0], swing_data_len)
            swing_foot_pos[:,1] = np.linspace(LFPC_model.right_foot_pos[1], right_foot_target_pos[1], swing_data_len)
            swing_foot_pos[1:swing_data_len-1, 2] = 0.1
        else:
            left_foot_target_pos = [LFPC_model.p_x2, LFPC_model.p_y2, 0]
            swing_foot_pos[:,0] = np.linspace(LFPC_model.left_foot_pos[0], left_foot_target_pos[0], swing_data_len)
